...
func (r *runner) OnError(err error) {
	fmt.Println(r.role, "runner error:", err)
	r.errored = true
	(*r.client).Close()
	(*r.server).Close()
}
...
func Fuzz(data []byte) int {
	...
	go func() {
		...
		server.RunHandshake()
		fmt.Println("server handshake returned")
	...
	go func() {
		...
		client.RunHandshake()
		fmt.Println("client handshake returned")
	}()
	...
	for {
		...
		case c := <-cChunkChan:
			...
			if len(b) > 0 && b[0] == messageToReplace {
				...
				fmt.Printf("sending %s message (%d bytes) to server\n", messageType(b[0]).String(), len(b))
			}
			server.HandleMessage(b, encLevel)
			fmt.Println("\thandling message (to server) returned")
		case c := <-sChunkChan:
			b := c.data
			encLevel := c.encLevel
			if len(b) > 0 && b[0] == messageToReplace {
				...
				fmt.Printf("sending %s message (%d bytes) to client\n", messageType(b[0]).String(), len(b))
			}
			client.HandleMessage(b, encLevel)
			fmt.Println("\thandling message (to client) returned")
		case <-done: // test done
